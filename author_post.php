        ...
        <div class="col-md-8">
            ...
            if (isset($_GET['author'])) {
                ...
                if (isset($_SESSION['user_role']) && $_SESSION['user_role'] === 'admin') {
                    $post_count_query = "SELECT * FROM posts WHERE post_author = '$author'";
                    ...
                    $query = "SELECT * FROM posts WHERE post_author = '$author' ORDER BY post_id DESC LIMIT $page_1, $per_page";
                    $selectAllPosts = mysqli_query($connection, $query);
                } else {
                    $post_count_query = "SELECT * FROM posts WHERE post_author = '$author' AND post_status = 'published'";
                    $find_count = mysqli_query($connection, $post_count_query);
                    $post_count = (mysqli_num_rows($find_count));
                    $post_count = ceil($post_count / $per_page);
            ...
            <ul class="pagination">
                ...
                for ($i = 1; $i <= $post_count; $i++) {
                    if ($i == $page || ($i == 1 && $page == null)) {
                        echo "<li class='active'><a href='author_post.php?author=$author&page=$i'>$i</a></li>";
                    } else {
                        echo "<li><a href='author_post.php?author=$author&page=$i'>$i</a></li>";
                    }
                }
                ?>
